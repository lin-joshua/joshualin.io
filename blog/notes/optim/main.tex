...
\begin{concept}[Momentum]
    ...
    \end{align*}
    It is equivalent to \textit{heavy-ball momentum}, \(w_{k+1} = w_k - \alpha \nabla f(w_k) + \beta (w_k - w_{k-1})\), and reduces to gradient descent when \(\beta = 0\). Intuitively, in regions of low curvature, momentum mitigates stalled training, while in regions of high curvature, it smooths out fluctuations. 
\end{concept}

\begin{remark}
    Algebraic equivalence to the heavy-ball form follows from taking
    \[
        w_{k+1} = w_k - \alpha g_{k+1}
        \implies g_{k+1} = \frac{w_{k+1} - w_k}{-\alpha},\, g_k = \frac{w_k - w_{k-1}}{-\alpha}
    \]
    from which it follows
    \begin{align*}
        g_{k+1} = \beta g_k + \nabla f(w_k)
        &\implies \frac{w_{k+1} - w_k}{-\alpha} = \beta \frac{w_k - w_{k-1}}{-\alpha} + \nabla f(w_k) \\
        &\implies w_{k+1} = w_k - \alpha \nabla f(w_k) + \beta (w_k - w_{k-1})
    \end{align*}
...
\begin{remark}
    ...
    \[
        v_{k+1} = v_k - \alpha \Lambda Q^\top (w_k - w^*) = v_k - \alpha \Lambda v_k
        \implies 
    ...
    \]
    where \(\kappa = \lambda_1 / \lambda_n \geq 1\) is the condition number of \(A\). In comparison, consider the momentum updates under a similar change of basis \(x_k = Q^\top g_k\) and \(v_k = Q^\top (w_k - w^*)\), 
    \begin{align*}
        x_{k+1} &= \beta x_k + \Lambda v_k \\
        v_{k+1} &= v_k - \alpha x_{k+1} = (I - \alpha \Lambda) v_k - \alpha \beta x_k
    \end{align*}
