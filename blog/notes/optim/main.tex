...
\begin{remark}
    There is an operator-theoretic representation: view \(V^\pi \in \mathbb{R}^S\) and \(Q^\pi \in \mathbb{R}^{SA}\), and define \(\mathbb{P} \in \mathbb{R}^{SA \times S}\) to be the transition dynamics \(\mathbb{P}_{(s,a), s'} = \mathbb{P}(s' \mid s, a)\). Let \(\Pi_\pi \in \mathbb{R}^{S \times SA}\) be the policy matrix with \((\Pi_\pi)_{s,(s',a)} = \mathbf{1}[s = s'] \pi(a \mid s)\), and let \(r \in \mathbb{R}^{SA}\) be the reward vector. Then the Bellman consistency equations become
    \[
        Q^\pi = r + \gamma \mathbb{P} \Pi_\pi Q^\pi, \qquad V^\pi = \Pi_\pi Q^\pi
    \]
    Define the \textit{Bellman evaluation operator} \(\mathcal{T}^\pi : \mathbb{R}^{SA} \to \mathbb{R}^{SA}\) by \(\mathcal{T}^\pi Q = r + \gamma \mathbb{P} \Pi_\pi Q\). Then \(Q^\pi\) is the unique fixed point of \(\mathcal{T}^\pi\), and since \(\mathcal{T}^\pi\) is a \(\gamma\)-contraction in \(\|\cdot\|_\infty\), the solution is
    \[
        Q^\pi = (I - \gamma \mathbb{P}\Pi_\pi)^{-1} r
    \]
    Similarly, the \textit{Bellman optimality operator} \(\mathcal{T}^* Q = r + \gamma \mathbb{P} \max_\pi \Pi_\pi Q\) has unique fixed point \(Q^*\), and the optimal policy is \(\pi^*(s) = \argmax_a Q^*(s,a)\). Value iteration is precisely the repeated application \(Q_{k+1} = \mathcal{T}^* Q_k\), which converges at rate \(\gamma^k\).
\end{remark}

\begin{concept}[Temporal Difference Learning]
    TD learning updates the value function using \textit{bootstrapped} estimates rather than waiting for complete trajectory returns. The \textbf{TD(0)} update rule is
    \[
        V(s_t) \leftarrow V(s_t) + \alpha \underbrace{\left(r(s_t, a_t) + \gamma V(s_{t+1}) - V(s_t)\right)}_{\delta_t \text{ (TD error)}}
    \]
    More generally, \textbf{TD(\(\lambda\))} interpolates between TD(0) and Monte Carlo via eligibility traces. Define the \(n\)-step return
    \[
        G_t^{(n)} = \sum_{k=0}^{n-1} \gamma^k r_{t+k} + \gamma^n V(s_{t+n})
    \]
    Then the \(\lambda\)-return is an exponentially-weighted average of all \(n\)-step returns,
    \[
        G_t^\lambda = (1 - \lambda) \sum_{n=1}^{T-t} \lambda^{n-1} G_t^{(n)}
    \]
    TD(0) is biased but low-variance (it bootstraps from \(V\), which may be inaccurate), while Monte Carlo (\(\lambda = 1\)) is unbiased but high-variance. TD methods are \textit{off-policy compatible} and converge under standard stochastic approximation conditions (Robbins-Monro). With function approximation \(V_\phi\), the update becomes semi-gradient:
    \[
        \phi \leftarrow \phi + \alpha \delta_t \nabla_\phi V_\phi(s_t)
    \]
    where crucially the gradient is \textit{not} taken through the bootstrap target \(V_\phi(s_{t+1})\).
\end{concept}
